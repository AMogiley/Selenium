...
class Search(unittest.TestCase):
	...
	def test(self):
		...
		self.driver.find_element_by_xpath('//*[@id="lb"]/div/a[1]').click()	# зайти во вкладку картинки через кнопку ещё (почему то версия страницы google переодически менялась, но потом стала выходить только страница где вкладка картинки открывается через кнопку Ещё)
